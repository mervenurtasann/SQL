...
-- 1. Adim : Personel sayisi 15.000'den buyuk olan sirketleri listeleyelim
...
----------------------------------------------------------------*/
SELECT sirket_id,calisanlar_sayisi,sirket
FROM sirketler
WHERE sirket IN (SELECT sirket
				FROM calisanlar
                WHERE sehir='Ankara');
...
SELECT sirket_id, sirket, (SELECT COUNT(sehir)
						   FROM calisanlar
						   WHERE sirketler.sirket = calisanlar.sirket) sehir_sayisi
FROM sirketler;
...
SELECT sirket_id, sirket, (SELECT COUNT(sehir)
						   FROM calisanlar
						   WHERE sirketler.sirket = calisanlar.sirket) toplam_sehir
FROM sirketler
WHERE sirket_id>101;
